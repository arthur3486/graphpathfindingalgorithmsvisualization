...
public class AStarAlgorithm extends BaseAlgorithm {
	...
	public static AStarAlgorithm fromGraph(BaseGraph graph, Vertex initialNode, Vertex destinationNode) {
		return new AStarAlgorithm(graph, initialNode, destinationNode);
	}
	
	
	
	
	private AStarAlgorithm(BaseGraph graph, Vertex initialNode, Vertex destinationNode) {
	...
	protected void prepareNodes() {
		...
		//(if the Vertex is equal to the Initial One, then we set its tentative distance to 0)
		for(Vertex vertex : mGraph.getVertices()) {
			if(vertex.equals(mInitialNode)) {
				vertex.setGCost(0);
				vertex.setHCost(mInitialNode.distance(mDestinationNode));
				vertex.setPreviousVertex(null);
				mUnvisitedNodes.push(vertex);
			} else {
				mUnvisitedNodes.push(vertex.setGCost(Double.POSITIVE_INFINITY).setPreviousVertex(null));
			}
		}
	}
		...
		while(!mUnvisitedNodes.isEmpty()) {
			...
			for(Vertex neighbor : neighbors) {
				...
				if(calculatedDistance < neighbor.getGCost()) {
					...
					neighbor.setHCost(neighbor.heuristicDistance(mDestinationNode));
